// src/app/summitblog/summit-insights/[slug]/page.tsx
import SectionHero from "@/components/generalHero/SectionHero";
import DefaultLayout from "@/components/layout/DefaultLayout";
import { summitInsights } from "@/data/summitInsights";
import Link from "next/link";
import React from "react";
import { notFound } from "next/navigation";

interface SummitInsightPageProps {
  params: Promise<{ slug: string }>;
}

export default async function SummitInsightPage({ params }: SummitInsightPageProps) {
  const { slug } = await params; // unwrap the promise

  const post = summitInsights.find((p) => p.slug === slug);
