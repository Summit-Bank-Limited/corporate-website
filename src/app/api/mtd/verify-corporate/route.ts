...
export async function POST(request: NextRequest) {
  try {
    ...
    const { accountNumber, tin } = body;

    // Validate required fields
    if (!accountNumber || !tin) {
      return NextResponse.json(
        { success: false, error: 'Account number and TIN are required' },
        { status: 400 }
      );
    }

    const response = await fetch('https://products.summitbankng.com/mtd/mtd/verify-corporate', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ accountNumber, tin }),
    });
    ...
    console.error('Error verifying corporate account:', error);
    return NextResponse.json(
      { success: false, error: 'Failed to verify corporate account' },
      { status: 500 }
    );
  }
}
